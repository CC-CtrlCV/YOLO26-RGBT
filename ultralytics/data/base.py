...
class BaseDataset(Dataset):
    ...
    def __init__(
        ...
        self.npy_files = [Path(f).with_name(f"{Path(f).stem}{'_rgbt' if self.rgbt else ''}.npy") for f in self.im_files]
        ...
        if im is None:  # not cached in RAM
            if fn.exists():  # load npy
                try:
                    ...
                    im = self._read_image(f)  # BGR or RGBT
            else:  # read image
                im = self._read_image(f)  # BGR or RGBT
            if im is None:
    ...
    def _paired_ir_path(self, rgb_path: str | Path) -> str:
        """Return the infrared path paired with a visible image path."""
    ...
    def _read_image(self, file: str | Path) -> np.ndarray | None:
        """Read a regular image or merge a visible/infrared pair into a 4-channel RGBT image."""
        if not self.rgbt:
            return imread(file, flags=self.cv2_flag)

        im_rgb = imread(file, flags=cv2.IMREAD_COLOR)
        ir_file = self._paired_ir_path(file)
        im_ir = imread(ir_file, flags=cv2.IMREAD_GRAYSCALE)
        if im_rgb is None or im_ir is None:
            raise FileNotFoundError(f"RGBT image pair not found: {file} | {ir_file}")
        if im_ir.ndim == 3:
            im_ir = im_ir[..., 0]
