...
def _validate_rgbt_pair(rgbt_pair: list[str] | tuple[str, str]) -> tuple[str, str]:
    """Validate visible/infrared path tokens used to locate paired RGBT images."""
    if (
        isinstance(rgbt_pair, (list, tuple))
        and len(rgbt_pair) == 2
        and all(isinstance(x, str) and x for x in rgbt_pair)
    ):
        return rgbt_pair[0], rgbt_pair[1]
    return "visible", "infrared"


def _read_rgbt_image(path: str | Path, rgbt_pair: tuple[str, str]) -> np.ndarray:
    """Read a visible image and its paired infrared image, returning a 4-channel BGR+T array."""
    rgb_token, ir_token = rgbt_pair
    path = str(path)
    ...
    ir_path = path.replace(rgb_token, ir_token, 1)
    im_rgb = imread(path, flags=cv2.IMREAD_COLOR)
    im_ir = imread(ir_path, flags=cv2.IMREAD_GRAYSCALE)
    if im_rgb is None or im_ir is None:
        raise FileNotFoundError(f"RGBT image pair not found: {path} | {ir_path}")
    if im_ir.ndim == 3:
        im_ir = im_ir[..., 0]
    if im_rgb.shape[:2] != im_ir.shape[:2]:
        im_ir = cv2.resize(im_ir, (im_rgb.shape[1], im_rgb.shape[0]), interpolation=cv2.INTER_LINEAR)
    return np.dstack((im_rgb, im_ir[..., None]))
...
class LoadImagesAndVideos:
    ...
    def __init__(
        ...
        self.bs = batch
        self.rgbt = bool(rgbt)
        self.rgbt_pair = _validate_rgbt_pair(rgbt_pair)
        self.cv2_flag = cv2.IMREAD_GRAYSCALE if channels == 1 else cv2.IMREAD_COLOR  # grayscale or color (BGR)
        if self.rgbt and nv:
            raise NotImplementedError("RGBT prediction currently supports image files/directories, not video sources.")
        if any(videos):
    ...
    def __next__(self) -> tuple[list[str], list[np.ndarray], list[str]]:
        ...
        while len(imgs) < self.bs:
            ...
            if self.video_flag[self.count]:
                ...
                # Handle image files
                self.mode = "image"
                im0 = _read_rgbt_image(path, self.rgbt_pair) if self.rgbt else imread(path, flags=self.cv2_flag)  # BGR
                if im0 is None:
